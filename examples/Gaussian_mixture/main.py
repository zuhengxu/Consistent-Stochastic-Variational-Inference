...
def get_init(arguments):
    ...
    K,D, alpha0 = eval(dim_dict[arguments.dataset])
    ...
    for i in range(20):
        ...
        mu_random = prior_sample(1, K, D, alpha0)[0]
        mu_init_dict = {'Prior': 'mu_random',
                        'SMAP': 'SMAP(mu_random)',
                        'SMAP_adam': 'SMAP_AD(mu_random)'}
        # initialization for L
        L_init_dict= {'Random': np.diag(np.exp(np.random.random(15)*np.log(10)+ np.log(0.1)) ),
                    'Ind': np.identity(K*(2*D+1))}
...
def run_vi(arguments):
    ...
    for k in range(size):
        ...
        x = vi_alg(init_val, N , lpdf, vi_lrt, 100000)
        # compute elbo
...
parser.add_argument('--alpha', type = int, default= 1 , help = 'Smoothing variance')
...
get_init_subparser.add_argument('--map_stepsched', type = str, default= 'lambda itr: 0.1', help = 'MAP initialization step schedule')
