...
df_syn_elbo = syn_elbo.melt(value_vars=['CSVI(adam)_Ind', 'CSVI(adam)_Rand', 'SVI(adam)_Ind', 'SVI(adam)_Rand'], 
...
df_real_elbo = real_elbo.melt(value_vars=['CSVI(adam)', 'CSVI(adam)_Rand', 'SVI(adam)_Ind', 'SVI(adam)_Rand'], 
                        var_name='method', value_name= 'ELBO', ignore_index = True) 
print(df_real_elbo)
...
ax2 = sns.violinplot(x = 'method', y = 'ELBO',data = df_real_elbo, 
                    scale = 'count', inner = 'stick', bw = 0.1,
                    order = ['CSVI(adam)', 'CSVI(adam)_Rand' , 'SVI(adam)_Ind', 'SVI(adam)_Rand'])
plt.xlabel('')
plt.ylabel('ELBO', fontsize =18)
